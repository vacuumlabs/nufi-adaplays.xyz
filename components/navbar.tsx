...
const ConnectButton = () => {
  ...
  // Ensure that widget is shown on page refresh if authenticated against it
  useEffect(() => {
    const fn = async () => {
  ...
  useEffect(() => {
    // Set timeout is used as `useEffect` in `_app` runs sooner. This seems to be
    // NextJS specific requirement.
    setTimeout(() => {
    ...
    <>
      ...
      <Popover onClose={resetStatus}>
        <PopoverTrigger>
          ...
          dapp is designed to work this way. Should not be needed in other apps. */}
